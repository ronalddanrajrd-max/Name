...
class RedeemModal(discord.ui.Modal, title="Redeem Key"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if row["used_by"] and row["used_by"] != user_id:
            return await interaction.response.send_message(
                embed=error_embed("Key Used", "This key is already linked to another user."),
                ephemeral=True
            )
...
class WhitelistPanel(discord.ui.View):
    ...
    @discord.ui.button(label="🔑 Redeem Key", style=discord.ButtonStyle.success, custom_id="okvehub_redeem_key_final_v10")
    async def redeem_key(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(RedeemModal())

    @discord.ui.button(label="📜 Get Script", style=discord.ButtonStyle.primary, custom_id="okvehub_get_script_final_v10")
    ...
    @discord.ui.button(label="👤 Get Role", style=discord.ButtonStyle.primary, custom_id="okvehub_get_role_final_v10")
    async def get_role(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_message(
            embed=success_embed("Roles Given", "Your roles were added."),
            ephemeral=True
        )

    @discord.ui.button(label="⚙️ Reset HWID", style=discord.ButtonStyle.secondary, custom_id="okvehub_reset_hwid_final_v10")
    ...
    @discord.ui.button(label="📊 Get Stats", style=discord.ButtonStyle.secondary, custom_id="okvehub_stats_final_v10")
    async def get_stats(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed.description = (
            ...
            f"**Script:** `{wl['script_access'] or DEFAULT_SCRIPT}`\n"
            f"**Expires At:** `{wl['expires_at'] or 'Never'}` 📅\n"
...
class Whitelist(commands.Cog):
    ...
    async def wl_add(self, interaction: discord.Interaction, utilisateur: discord.Member, script: str = DEFAULT_SCRIPT, jours: int = 0):
    ...
    async def wl_remove(self, interaction: discord.Interaction, utilisateur: discord.Member):
        ...
        await interaction.response.send_message(
            embed=success_embed("Whitelist retirée", f"{utilisateur.mention} n'est plus whitelist et sa key a été supprimée."),
            ephemeral=True
        )
    ...
    async def wl_check(self, interaction: discord.Interaction, utilisateur: discord.Member = None):
        ...
        embed.add_field(name="Script", value=row["script_access"] or DEFAULT_SCRIPT, inline=True)
    ...
    async def wl_list(self, interaction: discord.Interaction):
        ...
        lines = [f"`{i}.` <@{row['user_id']}> — **{row['script_access']}**" for i, row in enumerate(rows[:20], 1)]
    ...
    async def wl_blacklist(self, interaction: discord.Interaction, utilisateur: discord.Member, raison: str):
        ...
        await db_execute("DELETE FROM whitelist WHERE user_id=?", (str(utilisateur.id),))
        await db_execute("UPDATE keys SET status='disabled' WHERE used_by=?", (str(utilisateur.id),))
        await remove_roles(utilisateur)
